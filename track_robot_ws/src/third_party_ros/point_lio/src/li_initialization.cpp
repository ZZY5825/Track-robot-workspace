...
// sensor_msgs::Imu::ConstSharedPtr imu_last_ptr;
...
std::deque<sensor_msgs::Imu::SharedPtr> imu_deque;
...
void standard_pcl_cbk(const sensor_msgs::PointCloud2::ConstSharedPtr &msg) 
{
    ...
    if (ros::toSec(msg->header.stamp) < last_timestamp_lidar)
    {
    ...
    if (con_frame)
    {
        if (frame_ct == 0)
        {
            time_con = last_timestamp_lidar; //ros::toSec(msg->header.stamp);
        }
        if (frame_ct < 10)
        {
    ...
    { 
        if (ptr->points.size() > 0)
        {
            ...
            time_buffer.emplace_back(ros::toSec(msg->header.stamp));
        }
    }
    }
...
void imu_cbk(const sensor_msgs::Imu::ConstSharedPtr &msg_in) 
{
    ...
    // publish_count ++;
    sensor_msgs::Imu::SharedPtr msg(new sensor_msgs::Imu(*msg_in));

    msg->header.stamp = ros::Time().fromSec(
        ros::toSec(msg->header.stamp) - time_diff_lidar_to_imu
        - timediff_imu_wrt_lidar - time_lag_IMU_wtr_lidar);

    double timestamp = ros::toSec(msg->header.stamp);
...
bool sync_packages(MeasureGroup &meas)
{
    ...
    { 
        ...
        {
            double imu_time = ros::toSec(imu_deque.front()->header.stamp);
            ...
            while (imu_time < lidar_end_time)
            {
                ...
                imu_time = ros::toSec(imu_deque.front()->header.stamp); // can be changed
                imu_next = *(imu_deque.front());
            }
        }
        imu_pushed = true;

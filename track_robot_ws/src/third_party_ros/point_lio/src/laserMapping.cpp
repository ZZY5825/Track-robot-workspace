...
int main(int argc, char** argv)
{
    ...
    {
        ROS_WARN("Livox CustomMsg input is not enabled in this ROS2 port; subscribing as PointCloud2.");
    }
    ros::Subscriber sub_pcl = nh.subscribe<sensor_msgs::PointCloud2>(lid_topic, 200000, standard_pcl_cbk);
    ...
    while (status)
    {
        ...
        if(sync_packages(Measures)) 
        {
            ...
            if (flg_first_scan)
            {
                ...
                if (first_imu_time < 1)
                {
                    first_imu_time = ros::toSec(imu_next.header.stamp);
                ...
                if(imu_en)
                {
                    ...
                    {
                        while (Measures.lidar_beg_time > ros::toSec(imu_next.header.stamp)) // if it is needed for the new map?
                        {
                            imu_deque.pop_front();
                            if (imu_deque.empty())
                            {
            ...
            // These vectors are indexed below, so capacity alone is insufficient.
            ...
            {     
                ...
                for (k = 0; k < time_seq.size(); k++)
                {
                    ...
                    if (is_first_frame)
                    {
                        if(imu_en)
                        {
                            while (time_current > ros::toSec(imu_next.header.stamp))
                            {
                                imu_deque.pop_front();
                    ...
                    {
                        bool last_imu = ros::toSec(imu_next.header.stamp) == ros::toSec(imu_deque.front()->header.stamp);
                        while (ros::toSec(imu_next.header.stamp) < time_predict_last_const && !imu_deque.empty())
                        {
                            if (!last_imu)
                            {
                                imu_last = imu_next;
                        ...
                        bool imu_comes = time_current > ros::toSec(imu_next.header.stamp);
                        while (imu_comes) 
                        {
                            imu_upda_cov = true;
                            ...
                            /*** covariance update ***/
                            double dt = ros::toSec(imu_next.header.stamp) - time_predict_last_const;
                            kf_output.predict(dt, Q_output, input_in, true, false);
                            time_predict_last_const = ros::toSec(imu_next.header.stamp); // big problem
                            
                            {
                                double dt_cov = ros::toSec(imu_next.header.stamp) - time_update_last; 

                                if (dt_cov > 0.0)
                                {
                                    time_update_last = ros::toSec(imu_next.header.stamp);
                            ...
                            imu_last = imu_next;
                            imu_next = *(imu_deque.front());
                            imu_comes = time_current > ros::toSec(imu_next.header.stamp);
                        }
                    }
                    if (flg_reset)
                    {
                ...
                else
                {
                    ...
                    while (ros::toSec(imu_next.header.stamp) > time_current && ((ros::toSec(imu_next.header.stamp) < Measures.lidar_beg_time + lidar_time_inte )))
                    { // >= ?
                        if (is_first_frame)
                        {
                            {
                                {
                                    while (ros::toSec(imu_next.header.stamp) < Measures.lidar_beg_time + lidar_time_inte)
                                    {
                        ...
                        time_current = ros::toSec(imu_next.header.stamp);
            ...
            else
            {
                ...
                for (k = 0; k < time_seq.size(); k++)
                {
                    ...
                    while (time_current > ros::toSec(imu_next.header.stamp)) // && !imu_deque.empty())
                    {
                        imu_deque.pop_front();
                        ...
                        input_in.acc <<imu_last.linear_acceleration.x, imu_last.linear_acceleration.y, imu_last.linear_acceleration.z; 
                        input_in.acc    = input_in.acc * G_m_s2 / acc_norm; 
                        double dt = ros::toSec(imu_last.header.stamp) - t_last;

                        double dt_cov = ros::toSec(imu_last.header.stamp) - time_update_last;
                        if (dt_cov > 0.0)
                        {
                            kf_input.predict(dt_cov, Q_input, input_in, false, true); 
                            time_update_last = ros::toSec(imu_last.header.stamp); //time_current;
                        }
                        kf_input.predict(dt, Q_input, input_in, true, false); 
                        t_last = ros::toSec(imu_last.header.stamp);
                        imu_prop_cov = true;
                ...
                else
                {
                    ...
                    { // >= ?
                        ...
                        {        
                            // kf_input.predict(dt_cov, Q_input, input_in, false, true);
                            time_update_last = ros::toSec(imu_next.header.stamp); //time_current;
                        }
                        // kf_input.predict(dt, Q_input, input_in, true, false);

                        t_last = ros::toSec(imu_next.header.stamp);

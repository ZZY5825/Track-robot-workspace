...
void Preprocess::process_cut_frame_livox(const livox_ros_driver::CustomMsg::ConstPtr &msg,
    ...
    double last_frame_end_time = ros::toSec(msg->header.stamp) * 1000;
    uint valid_num = 0;
    uint cut_num = 0;
    ...
    for (uint i = 1; i < valid_pcl_size; i++) {
        ...
        pl_surf[i].curvature += ros::toSec(msg->header.stamp) * 1000 - last_frame_end_time;
...
void
Preprocess::process_cut_frame_pcl2(const sensor_msgs::PointCloud2::ConstSharedPtr &msg, deque<PointCloudXYZI::Ptr> &pcl_out,
    ...
    if (lidar_type == VELO16) {
        ...
        for (int i = 0; i < plsize; i++) {
            ...
            added_pt.z = pl_orig.points[i].z;
            added_pt.intensity = pl_orig.points[i].intensity;
            added_pt.curvature = (pl_orig.points[i].timestamp - ros::toSec(msg->header.stamp)) * 1000;  //s to ms
...
void Preprocess::oust64_handler(const sensor_msgs::PointCloud2::ConstSharedPtr &msg)
{
  ...
  double time_stamp = ros::toSec(msg->header.stamp);
  // cout << "===================================" << endl;
...
void Preprocess::hesai_handler(const sensor_msgs::PointCloud2::ConstSharedPtr &msg)
{
    pl_surf.clear();
    pl_corn.clear();
    pl_full.clear();
